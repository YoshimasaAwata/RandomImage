...
def on_ui_tabs():
    ...
    with gr.Blocks(css="scripts/rdmimg/random_image.css") as random_image:
        ...
        with gr.Row():
            with gr.Column():
                with gr.Tab(label="Smooth") as smooth_tab:
                    tile_size_rdo = gr.Radio(
                        [2, 4, 8, 16, 32], value=4, label="Tile size"
                    )
                ...
                image_color_rdo = gr.Radio(
                    ["RGB", "GRAYSCALE"], value="RGB", label="Color"
                )
                with gr.Row():
        ...
        seed_clear_btn.click(
            lambda: (gr.Number.update(value=-1)), outputs=rand_seed_num
        )
    ...
    return [(random_image, "RandomImage", "RandomImage")]
